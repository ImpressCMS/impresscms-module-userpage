...
class PDF extends PDF_language
{
	...
	//    $html=strip_tags($html,"<b><u><i><a><img><p><br><strong><em><font><tr><blockquote>"); //remove all unsupported tags
	    ...
	    foreach($a as $i=>$e)
	    {
	        if($i%2==0)
	        {
	            ...
	            elseif($this->IMG) $this->PutImage($this->SRC,$scale);
			    elseif($this->CENTER) $this->Cell(0,5,$e,0,1,'C');
			    elseif($this->ALIGN == 'center') $this->Cell(0,5,$e,0,1,'C');
			    elseif($this->ALIGN == 'right') $this->Cell(0,5,$e,0,1,'R');
			    elseif($this->ALIGN == 'left') $this->Cell(0,5,$e,0,1,'L');
	...
	function PutImage($url,$scale)
	{
		...
		if($scale>0){
			...
			//get image info
			$oposy=$this->GetY();
			$url = str_replace(ICMS_URL, ICMS_ROOT_PATH, $url);
